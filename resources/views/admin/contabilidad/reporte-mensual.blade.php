...
<head>
    ...
    <title>Reporte mensual - {{ $nombreMes }} - Funeraria San José</title>
    <style>
        * { box-sizing: border-box; }
        ...
        @media print {
            ...
            .reporte-footer { border-top-color: #FFD700; }
        }
    </style>
</head>
<body>
    <div class="reporte-wrap">
        ...
        <div class="reporte-body">
            <p class="reporte-periodo">Reporte de entradas — Período: {{ $nombreMes }}</p>

            <h2>Detalle de pagos aprobados</h2>
            <table>
                <thead>
                    <tr>
                        <th>Fecha</th>
                        <th>Afiliado / Usuario</th>
                        <th>Plan / Concepto</th>
                ...
                <tbody>
                    @forelse($pagos as $p)
                        <tr>
                            ...
                            <td>{{ $p->afiliado?->user?->name ?? '—' }}</td>
                            <td>{{ $p->planExequial?->nombre ?? 'Pago' }}</td>
                            <td class="text-right">${{ number_format((float) $p->monto, 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="4" class="text-right">No hay pagos en este período.</td></tr>
                    @endforelse
                </tbody>
                <tfoot>
                    <tr class="total">
                        <td colspan="3" class="text-right">Total entradas</td>
                        <td class="text-right">${{ number_format($totalEntradas, 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </table>
